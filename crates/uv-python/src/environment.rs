...
impl PythonEnvironment {
    ...
    ///
    /// Returns the path that should be used to access the environment:
    /// - The link path if a symlink/junction was created (traversable for activation)
    /// - The real root if a file fallback was used or the environment is not centralized
